...
echo "[INFO] Starting Drone Demo with NOVATEL GPS"
echo "[INFO] Starting ROSCORE"
screen -m -d -S roscore bash -c roscore
sleep 4
echo "[INFO] Nissan Bringup"
screen -m -d -S nissan_bringup bash -c 'roslaunch nissan_bringup obstacle_sensors_nova.launch'
...
echo "[INFO] Starting Drone Topic"
ssh nvidia@192.168.1.30 'screen -d -m -S sensors ./start_sensors.sh'
echo "[INFO] Starting remote RVIZ"
sleep 3
ssh jkk@192.168.1.2 'screen -d -m -S viz_obstacle ./start_viz_obstacle_drone.sh'
